...
void *loop(void *arg) {
  ...
  for (;;) {
    ...
    screen[rect.start.y + rect.height / 2][rect.start.x + rect.width / 2] =
        EMPTY_CHAR;
    screen[rect.start.y + rect.height / 2][rect.start.x + rect.width / 2].c =
        '1' - 1 + (color_index % COLORS_SIZE);
    screen[rect.start.y + rect.height / 2][rect.start.x + rect.width / 2]
        .style.fontColor = c.style.fontColor;
...
int main() {
  pthread_t loop_thread, render_thread, io_thread;
